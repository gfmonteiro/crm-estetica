...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const response = anamnesisResponsesRepository.findById(session.organizationId, id);
  if (!response) return NextResponse.json({ error: "Resposta não encontrada" }, { status: 404 });

  const form = anamnesisFormsRepository.findById(session.organizationId, response.formId);
